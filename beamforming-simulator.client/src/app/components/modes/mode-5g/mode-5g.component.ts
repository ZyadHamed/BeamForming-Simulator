...
import {
  ...
  LinkQuality, NetworkStateResult,
...
export class Mode5gComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  towers: Tower[] = []; users: MobileUser[] = [];
  selectedTowerId: string | null = null; selectedUserId: string | null = null;
  placingMode: 'tower' | 'user' | null = null;
  showCoverage = true; showGlobalInterference = false;
  ...
  ngAfterViewInit(): void {
    // Use requestAnimationFrame to wait for CSS layout to finish
    requestAnimationFrame(() => {
      const canvas = this.envCanvasRef.nativeElement;
      const W = canvas.offsetWidth || 800;
      const H = canvas.offsetHeight || 500;
      this.towers = [
        this.makeTower(W * 0.25, H * 0.35, 0),
        this.makeTower(W * 0.70, H * 0.35, 1),
      ];
      this.users = [
        this.makeUser(W * 0.40, H * 0.65, 0),
        this.makeUser(W * 0.60, H * 0.60, 1),
        this.makeUser(W * 0.50, H * 0.78, 2),
      ];
  ...
  private makeTower(x: number, y: number, idx: number): Tower {
    ...
    return {
      ...
      color: TOWER_COLORS[idx % TOWER_COLORS.length], sectorAngles: { Alpha: 0, Beta: 0, Gamma: 0 }
    };
  }
  ...
  onCanvasMouseDown(ev: MouseEvent): void {
    ...
    if (hitTower && !this.placingMode) {
      ...
      this.dragOffsetX = x - hitTower.x;
      this.dragOffsetY = y - hitTower.y;
    ...
    if (hitUser) {
      this.selectedUserId = hitUser.id;
  ...
  onCanvasMouseMove(ev: MouseEvent): void {
    ...
    const tower = this.towers.find(t => t.id === this.draggingTowerId);
    if (!tower) return;
  ...
  private syncUsersToBackend(): void {
    ...
    const payload: User5GRequest[] = this.users.map(u => ({
      user_id: u.id, x_m: u.x / PX_PER_METER, y_m: u.y / PX_PER_METER,
      allocated_frequency_mhz: 3500,
    }));
  ...
  private applyNetworkResult(result: NetworkStateResult): void {
    ...
    result.active_connections.forEach((link: LinkQuality) => {
      const user = this.users.find(u => u.id === link.user_id);
      if (user) {
    ...
    tsa.forEach((sectorMap, towerId) => {
      ...
      sectorMap.forEach((angles, sn) => { tower.sectorAngles[sn] = angles.reduce((a, b) => a + b, 0) / angles.length; });
      const first = [...sectorMap.keys()][0];
      if (first) tower.array.steeringAngle = tower.sectorAngles[first];
    });
    this.redrawPolar(); this.cdr.markForCheck();
  }
  ...
  redrawPolar(): void {
    setTimeout(() => {
      ...
      if (!tower) { ctx.fillStyle = '#aaa'; ctx.font = '11px IBM Plex Mono,monospace'; ctx.fillText('Select a tower', cx, cy); return; }
      this.sectorNames.forEach(sn => {
        const sd = tower.sectorAngles[sn] ?? 0, bo = SECTOR_BORESIGHTS[sn] ?? 0, col = SECTOR_COLORS[sn], ia = sn === this.activeSectorTab;
        const pat = this._computePolarPattern(tower.array.numElements, tower.array.elementSpacing, sd, bo);
        ctx.beginPath(); ctx.strokeStyle = col + (ia ? 'ff' : '55'); ctx.lineWidth = ia ? 2 : 1;
        pat.forEach(([ad, an], i) => { const r = R * Math.max(0, an), rd = ((ad - 90) * Math.PI) / 180; i === 0 ? ctx.moveTo(cx + r * Math.cos(rd), cy + r * Math.sin(rd)) : ctx.lineTo(cx + r * Math.cos(rd), cy + r * Math.sin(rd)); });
        ctx.closePath(); if (ia) { ctx.fillStyle = col + '1e'; ctx.fill(); } ctx.stroke();
        const ar = ((sd + bo - 90) * Math.PI) / 180;
      ...
      this.sectorNames.forEach((n, i) => { ctx.fillStyle = SECTOR_COLORS[n]; ctx.font = '8px IBM Plex Mono,monospace'; ctx.textAlign = 'left'; ctx.fillText(`${n}: θ=${(tower.sectorAngles[n] ?? 0).toFixed(1)}°`, 5, 11 + i * 12); });
    });
  }

  private _computePolarPattern(ne: number, sm: number, sd: number, bd: number): Array<[number, number]> {
    const lambda = 3e8 / 3.5e9, d = sm * 1e-3, sr = ((sd + bd) * Math.PI) / 180;
    const pts: Array<[number, number]> = []; let max = 0;
    for (let deg = 0; deg < 360; deg++) { const th = (deg * Math.PI) / 180; let re = 0, im = 0; for (let n = 0; n < ne; n++) { const pos = (n - (ne - 1) / 2) * d, ph = (2 * Math.PI * pos / lambda) * (Math.sin(th) - Math.sin(sr)); re += Math.cos(ph); im += Math.sin(ph); } const a = Math.sqrt(re * re + im * im); pts.push([deg, a]); if (a > max) max = a; }
  ...
  get activeSectorParams(): { label: string; value: string }[] {
    ...
    const el = tower.array.elements[0];
    ...
    return [
      ...
      { label: 'Steering Angle', value: `${angle.toFixed(2)}°` },
      { label: 'Global Pointing', value: `${(angle + boresight).toFixed(2)}°` },
      { label: 'Elements', value: `${tower.array.numElements}` },
      { label: 'Spacing', value: `${tower.array.elementSpacing} mm` },
      { label: 'Frequency', value: `${el?.frequency ?? 3500} MHz` },
      { label: 'Apodization', value: tower.array.apodizationWindow ?? 'hamming' },
      { label: 'SNR config', value: `${tower.array.snr ?? 100} dB` },
  ...
  private moveSelectedUser(): void {
    ...
    if (moved) { this.framesSinceSync++; if (this.framesSinceSync >= this.SYNC_EVERY_N_FRAMES) { this.framesSinceSync = 0; this.syncUsersToBackend(); } else { this.updateConnectionsLocal(); } }
  }

  private updateConnectionsLocal(): void {
    this.users.forEach(user => {
      const inRange = this.towers.filter(t => Math.hypot(t.x - user.x, t.y - user.y) <= t.range).sort((a, b) => Math.hypot(a.x - user.x, a.y - user.y) - Math.hypot(b.x - user.x, b.y - user.y));
  ...
  private drawGlobalInterference(ctx: CanvasRenderingContext2D, W: number, H: number): void {
    ...
    this.towers.forEach(tower => {
      ...
      for (let px = 0; px < W; px += step)for (let py = 0; py < H; py += step) {
        let sr = 0, si = 0;
        for (const ep of elements) { const dx = px - ep.x, dy = py - ep.y, dist = Math.sqrt(dx * dx + dy * dy) || 0.001, k = (2 * Math.PI * ep.config.frequency * 1e6) / 3e8, pr = (ep.config.phaseShift * Math.PI) / 180, dr = 2 * Math.PI * ep.config.frequency * 1e6 * ep.config.timeDelay * 1e-6, amp = (ep.config.intensity / 100) / dist, ph = k * dist - this.t * 3 - pr - dr; sr += amp * Math.cos(ph); si += amp * Math.sin(ph); }
        const intensity = Math.min(Math.sqrt(sr * sr + si * si) * 30, 1);
        if (intensity > 0.05) { const [r, g, b] = tower.color.match(/\w\w/g)!.map(h => parseInt(h, 16)); ctx.fillStyle = `rgba(${r},${g},${b},${intensity * 0.5})`; ctx.fillRect(px, py, step, step); }
      }
    });
  }
  ...
  private drawCityGrid(ctx: CanvasRenderingContext2D, W: number, H: number): void {
    ...
    [[1, 1], [3, 2], [5, 1], [2, 4], [6, 3], [4, 5], [7, 2], [1, 4]].forEach(([bx, by]) => { ctx.fillStyle = 'rgba(160,180,160,0.3)'; ctx.fillRect(bx * gs + 4, by * gs + 4, gs - 8, gs - 8); });
  }
  ...
  private drawBeams(ctx: CanvasRenderingContext2D): void {
    this.towers.forEach(tower => {
      this.users.filter(u => u.connectedTowerId === tower.id).forEach(user => {
        const dx = user.x - tower.x, dy = user.y - tower.y, dist = Math.hypot(dx, dy), angle = Math.atan2(dy, dx), halfW = Math.PI / Math.max(tower.array.numElements, 2) * 0.8;
        ...
        grad.addColorStop(0, user.color + Math.round(snrA * 255).toString(16).padStart(2, '0')); grad.addColorStop(1, user.color + '11');
        ctx.beginPath(); ctx.moveTo(tower.x, tower.y); ctx.arc(tower.x, tower.y, dist, angle - halfW, angle + halfW); ctx.closePath(); ctx.fillStyle = grad; ctx.fill();
        const dashOffset = (this.t * 20) % 20;
        ctx.beginPath(); ctx.strokeStyle = user.color + 'cc'; ctx.lineWidth = 1.5; ctx.setLineDash([6, 4]); ctx.lineDashOffset = -dashOffset; ctx.moveTo(tower.x, tower.y); ctx.lineTo(user.x, user.y); ctx.stroke(); ctx.setLineDash([]); ctx.lineDashOffset = 0;
        for (let ring = 0; ring < 3; ring++) { const r = ((this.t * 40 + ring * 30) % dist), rx = tower.x + (dx / dist) * r, ry = tower.y + (dy / dist) * r, alpha = 0.4 * (1 - r / dist); ctx.beginPath(); ctx.arc(rx, ry, 5, 0, Math.PI * 2); ctx.strokeStyle = user.color + Math.round(alpha * 255).toString(16).padStart(2, '0'); ctx.lineWidth = 1; ctx.stroke(); }
      });
    });
  }
  ...
  private drawTowers(ctx: CanvasRenderingContext2D): void {
    this.towers.forEach(tower => {
      const isSel = tower.id === this.selectedTowerId, isDrag = tower.id === this.draggingTowerId, pulse = isSel ? 1 + 0.15 * Math.sin(this.t * 6) : 1;
      if (isDrag) { ctx.shadowColor = 'rgba(0,0,0,0.25)'; ctx.shadowBlur = 12; }
      ctx.beginPath(); ctx.arc(tower.x, tower.y, 14 * pulse, 0, Math.PI * 2); ctx.fillStyle = tower.color + '22'; ctx.fill();
      ...
      ctx.moveTo(tower.x, tower.y - 10); ctx.lineTo(tower.x, tower.y + 8); ctx.moveTo(tower.x - 7, tower.y - 4); ctx.lineTo(tower.x + 7, tower.y - 4); ctx.moveTo(tower.x - 5, tower.y - 8); ctx.lineTo(tower.x + 5, tower.y - 8); ctx.stroke();
      ctx.fillStyle = tower.color; ctx.font = 'bold 9px IBM Plex Mono,monospace'; ctx.textAlign = 'center'; ctx.fillText(`BS${this.towers.indexOf(tower) + 1}`, tower.x, tower.y + 22);
      ...
      this.sectorNames.forEach(name => { const bo = SECTOR_BORESIGHTS[name] ?? 0, st = tower.sectorAngles[name] ?? 0, rad = ((bo + st - 90) * Math.PI) / 180; ctx.beginPath(); ctx.strokeStyle = SECTOR_COLORS[name] + (isSel ? 'dd' : '77'); ctx.lineWidth = isSel ? 2 : 1; ctx.moveTo(tower.x, tower.y); ctx.lineTo(tower.x + 26 * Math.cos(rad), tower.y + 26 * Math.sin(rad)); ctx.stroke(); });
      if (isSel) { let yOff = tower.y + 44; this.sectorNames.forEach(name => { ctx.fillStyle = SECTOR_COLORS[name]; ctx.font = '7px IBM Plex Mono,monospace'; ctx.textAlign = 'center'; ctx.fillText(`${name} θ=${(tower.sectorAngles[name] ?? 0).toFixed(1)}°`, tower.x, yOff); yOff += 10; }); }
    });
  }
  ...
  private drawUsers(ctx: CanvasRenderingContext2D): void {
    this.users.forEach(user => {
      ...
      if (isSel) { ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke(); ctx.strokeStyle = user.color + '66'; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(user.x, user.y, 14 * pulse, 0, Math.PI * 2); ctx.stroke(); }
      if (user.connectedTowerId) { ctx.fillStyle = '#34a853'; ctx.beginPath(); ctx.arc(user.x + 6, user.y - 6, 3, 0, Math.PI * 2); ctx.fill(); }
      ctx.fillStyle = user.color; ctx.font = 'bold 9px IBM Plex Mono,monospace'; ctx.textAlign = 'center'; ctx.fillText(user.label, user.x, user.y + 18);
      if (user.data_rate_mbps > 0) { ctx.fillStyle = '#333'; ctx.font = '7px IBM Plex Mono,monospace'; ctx.fillText(`${user.data_rate_mbps.toFixed(0)} Mbps`, user.x, user.y + 28); ctx.fillText(`SNR ${user.snr_db.toFixed(1)} dB`, user.x, user.y + 37); if (user.sector_name) { ctx.fillStyle = SECTOR_COLORS[user.sector_name] ?? user.color; ctx.fillText(user.sector_name, user.x, user.y + 46); } }
    });
  }
}
